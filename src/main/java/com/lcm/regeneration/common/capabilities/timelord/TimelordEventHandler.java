...
public class TimelordEventHandler {
    ...
    @SubscribeEvent
    public void onRegenerationExplosion(RegenerationEvent.RegenerationExplosionEvent event) {
    ...
    @SubscribeEvent
    public void onRegenerationStart(RegenerationStartEvent event) {
        EntityPlayer player = event.getEntityPlayer();
        if (player.world.isRemote)
            return;
    ...
    public void onHurt(LivingHurtEvent e) {
        ...
        if ((handler.getState() != CapabilityTimelord.RegenerationState.NONE || player.posY < 0 || handler.getRegensLeft() <= 0 || e.getSource() instanceof DebugCommand.NoRegenDamageSource) && !RegenConfig.dontLoseUponDeath) {
