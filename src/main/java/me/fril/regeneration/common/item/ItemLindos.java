            ...
            if (hasWater(stack)) {
                if (getAmount(stack) == 100) {
                    if (data.canRegenerate()) {
                        CapabilityRegeneration.getForPlayer(playerIn).triggerRegeneration();
                    } else {
                        data.receiveRegenerations(1);
                        CapabilityRegeneration.getForPlayer(playerIn).triggerRegeneration();
                    }
                } else {
                    PlayerUtil.sendMessage(playerIn, new TextComponentTranslation("regeneration.messages.empty_vial"), true);
                }
            } else {
                PlayerUtil.sendMessage(playerIn, new TextComponentTranslation("regeneration.messages.no_water"), true);
            }
        }
