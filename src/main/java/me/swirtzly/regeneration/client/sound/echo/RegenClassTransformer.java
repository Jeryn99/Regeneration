...
public class RegenClassTransformer implements IClassTransformer, Opcodes {
				...
				for (int i = 0; i < method.instructions.size(); ++i) {
					...
					if (node instanceof MethodInsnNode) {
						...
						if (methodNode.name.equals(setRotationAnglesMethod) && methodNode.desc.equals(setRotationAnglesDesc)) {
							...
							list.add(new MethodInsnNode(INVOKESTATIC, "me/swirtzly/regeneration/client/sound/echo/RegenClassTransformer", "renderBipedPre", "(Lnet/minecraft/client/model/ModelBiped;Lnet/minecraft/entity/Entity;FFFFFF)V", false));
							continue;
						}
					}
					...
					if (node.getOpcode() == RETURN) {
						...
						list.add(new VarInsnNode(FLOAD, 6));
						list.add(new VarInsnNode(FLOAD, 7));
						list.add(new MethodInsnNode(INVOKESTATIC, "me/swirtzly/regeneration/client/sound/echo/RegenClassTransformer", "renderBipedPost", "(Lnet/minecraft/client/model/ModelBiped;Lnet/minecraft/entity/Entity;FFFFFF)V", false));
					}
	...
	public static void renderBipedPre(ModelBiped model, Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
		if (entity == null)
			return;
        AnimationEvent.SetRotationAngles ev = new AnimationEvent.SetRotationAngles(entity, model, f, f1, f2, f3, f4, f5, AnimationEvent.ModelRotationType.PRE);
	...
	public static void renderBipedPost(ModelBiped model, Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
		if (entity == null)
			return;
        AnimationEvent.SetRotationAngles ev = new AnimationEvent.SetRotationAngles(entity, model, f, f1, f2, f3, f4, f5, AnimationEvent.ModelRotationType.POST);
		MinecraftForge.EVENT_BUS.post(ev);
	}
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals("play")) {
                this.insertBefore(method.instructions, this.invoke(SOURCE_LWJGL_NAME, "checkPitch"::equals), () -> {
                    ...
                    instructions.add(new MethodInsnNode(INVOKESTATIC, "me/swirtzly/regeneration/client/sound/echo/SoundReverbHandler", "onPlaySound", "(I)V", false));
                    return instructions;
                });
                return true;
            }
