...
public class LayerHands extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {


	private final IEntityRenderer livingEntityRenderer;

	public LayerHands(IEntityRenderer livingEntityRendererIn) {
	...
	private void renderHand(PlayerEntity player, HandSide handSide, EnumHandRenderType type) {
		...
		CapabilityRegeneration.getForPlayer(player).ifPresent((data) -> {
			...
			if (type == EnumHandRenderType.REGEN) {
				TypeHandler.getTypeInstance(data.getType()).getRenderer().renderHand(player, handSide, (LivingRenderer) livingEntityRenderer);
			}
