 ...
 * on 16/09/2018.
 */
public class LayerRegeneration extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {
    ...
    public LayerRegeneration(IEntityRenderer livingEntityRendererIn) {
    ...
    public void render(AbstractClientPlayerEntity player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        CapabilityRegeneration.getForPlayer(player).ifPresent((data) -> {
            ...
            if (data.isSyncingToJar()) {
                TypeHandler.getTypeInstance(TypeHandler.RegenType.FIERY).getRenderer().onRenderRegenerationLayer(type, (LivingRenderer) livingEntityRenderer, data, player, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale);
            }
        });
