...
public class LayerFuzz implements LayerRenderer<EntityPlayer> {
	...
	public void doRenderLayer(EntityPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		...
		Minecraft.getInstance().renderEngine.bindTexture(SKELETON_TEXTURES);
		...
		if (playerTexture != null) {
			Minecraft.getInstance().renderEngine.bindTexture(playerTexture);
		}
		...
		if (entitylivingbaseIn.world.rand.nextInt(3) == 1) {
			GlStateManager.translatef(0, entitylivingbaseIn.world.rand.nextInt(6) / 100.0f, 0);
		}
