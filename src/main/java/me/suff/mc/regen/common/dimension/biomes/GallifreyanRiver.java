...
public class GallifreyanRiver extends Biome {
    ...
    public GallifreyanRiver() {
        super((new Builder()).surfaceBuilder(SurfaceBuilder.DEFAULT, SurfaceBuilder.CONFIG_GRASS).precipitation(RainType.RAIN).biomeCategory(Category.RIVER).depth(-0.5F).scale(0.0F).temperature(6f).downfall(0.5F).waterColor(BiomeHelper.RED_WATER_COLOR).waterFogColor(BiomeHelper.RED_WATER_COLOR).parent(null));
