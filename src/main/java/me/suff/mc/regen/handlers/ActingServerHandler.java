...
import me.suff.mc.regen.network.MessagePlayRegenerationSound;
    ...
    public void onRegenTick(IRegeneration cap) {
        ...
        switch (cap.getState()) {
            case POST:
                ...
                break;
            case REGENERATING:
                float dm = Math.max(1, (player.world.getDifficulty().getDifficultyId() + 1) / 3F); // compensating for hard difficulty
