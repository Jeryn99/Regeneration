...
import me.suff.mc.regen.common.types.TypeFiery;
...
public class TypeFieryRenderer extends ATypeRenderer<TypeFiery> {
    ...
    public static void renderOverlay(EntityPlayer entityPlayer, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale, @Nullable ModelBase base) {
        ...
        if (base == null) {
            if (SkinChangingHandler.getSkinType((AbstractClientPlayer) entityPlayer, false) == SkinInfo.SkinType.STEVE) {
                LayerRegeneration.modelSteve.isChild = false;
                LayerRegeneration.modelSteve.render(entityPlayer, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
            } else {
                LayerRegeneration.modelAlex.isChild = false;
                LayerRegeneration.modelAlex.render(entityPlayer, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
            }
        } else {
    ...
    public boolean onAnimateRegen(AnimationContext animationContext) {
        ...
        return AnimationHandler.copyAndReturn(playerModel, true);
    }
