package me.suff.mc.regen.client.rendering.tile;

import me.suff.mc.regen.client.models.ModelHand;
        ...
        if (te.getHand().getItem() == RegenObjects.Items.HAND) {
            ...
            if (handType.equals("ALEX")) {
                ...
                if (te.getLindosAmont() >= 100) {
                    TypeFieryRenderer.renderOverlay(Minecraft.getMinecraft().player, 0, 0, 0, 0, 0, 0, 0.0625F, ALEX_ARM);
                }
            } else {
                STEVE_ARM.render(null, 0, 0, 0, 0, 0, 0.0625F);
                if (te.getLindosAmont() >= 100) {
                    TypeFieryRenderer.renderOverlay(Minecraft.getMinecraft().player, 0, 0, 0, 0, 0, 0, 0.0625F, STEVE_ARM);
                }
            }
        } else {
            TEXTURES.remove(te);
