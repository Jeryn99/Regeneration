...
public class RegenScheduledAction extends ScheduledAction {
    ...
    public boolean tick() {
        if (scheduledTick == -1)
            Regeneration.LOGGER.warn(player.getName() + ": Ticking finished/canceled ScheduledAction (" + transition + ")");
    ...
    public double getProgress() {
        if (scheduledTick == -1)
            Regeneration.LOGGER.warn(player.getName() + ": Querying progress of canceled/finished transition");
