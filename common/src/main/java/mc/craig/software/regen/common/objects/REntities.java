...
public class REntities {
    ...
    public static final RegistrySupplier<EntityType<Timelord>> TIMELORD = ENTITY_TYPES.register("timelord", () -> EntityType.Builder.of((EntityType.EntityFactory<Timelord>) (entityType, level) -> new Timelord(level), MobCategory.CREATURE).sized(0.6F, 1.95F).build(RConstants.MODID + ":weeping_angel"));
    ...
    public static final RegistrySupplier<EntityType<Laser>> LASER = ENTITY_TYPES.register("laser", () -> EntityType.Builder.of((EntityType.EntityFactory<Laser>) (entityType, level) -> new Laser(REntities.LASER.get(), level), MobCategory.MISC).sized(0.25F, 0.25F).updateInterval(10).clientTrackingRange(4).build(RConstants.MODID + ":level"));
