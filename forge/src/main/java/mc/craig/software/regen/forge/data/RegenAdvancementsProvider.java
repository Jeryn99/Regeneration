...
public class RegenAdvancementsProvider extends ForgeAdvancementProvider {
    ...
    public static class RegenAdvancements implements AdvancementGenerator {
        ...
        public void generate(HolderLookup.Provider arg, Consumer<Advancement> advancementConsumer, ExistingFileHelper existingFileHelper) {
            ...
            Advancement changeRefusal = Advancement.Builder.advancement().parent(rootAdvancement).display(RItems.FOB.get(), Component.translatable(getTitleTranslation("change_refusal")), Component.translatable(getDescriptionTranslation("change_refusal")), null, FrameType.TASK, false, true, false).addCriterion("change_refusal", new BaseTrigger.Instance(TriggerManager.CHANGE_REFUSAL.getId())).save(advancementConsumer, "regen/change_refusal");

            Advancement zeroStatus = Advancement.Builder.advancement().parent(firstRegeneration).display(RBlocks.ZERO_ROOM_FULL.get(), Component.translatable(getTitleTranslation("zero_status")), Component.translatable(getDescriptionTranslation("zero_status")), null, FrameType.TASK, false, true, false).addCriterion("zero_status", new BaseTrigger.Instance(TriggerManager.ZERO_ROOM.getId())).save(advancementConsumer, "regen/zero_status");
